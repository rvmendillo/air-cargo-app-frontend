...
def convert_xml_string_to_readable_json(xml_data):
    """
    Converts XML to standard, human-readable JSON.
    Strips namespaces, cleans whitespace, and removes metadata.
    """

    def clean_namespaces(path, key, value):
        # 1. Strip newlines and normalize whitespace for human readability
        if isinstance(value, str):
            value = " ".join(value.split())
        ...
        # 3. Strip @ from attributes (e.g., @schemeID -> schemeID)
        if key.startswith('@'):
            key = key[1:]
            
        # 4. Filter out XML boilerplate and metadata
        key_lower = key.lower()
        garbage_keys = ['xmlns', 'schemalocation', 'rsm', 'ccts', 'udt', 'ram', 'xsd', 'xsi']
    ...
    try:
        # Parse XML into a standard dictionary
        # cdata_key='value' renames #text to value
        # dict_constructor=dict makes it a standard Python dict instead of OrderedDict
